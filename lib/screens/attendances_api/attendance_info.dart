  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          !_loadingUser ? _calendarIcon(context) : const Text(''),
        ],
      ),
      body: _isLoadingCount || _loadingUser || _isLoadingCountAll
          ...
          : Column(
              children: [
                _filterWidget(local, context),
                _nameWidget(context, local),
                const SizedBox(
                  height: 15,
                ),
                _attendanceList(isEnglish, local),
              ],
            ),
    );
  }

// filter widget to filter attendance by day/multiday/all-time
  Padding _filterWidget(AppLocalizations? local, BuildContext context) {
    return Padding(
                    ...
                    Row(
                      ...
                      children: [
                        TextButton(
                          ...
                          onPressed: () {
                            if (sortByValue == '${local?.optionMultiDay}') {
                              setState(() {
                                isOneDay = false;
                                alltime = false;
                                now = false;
                            ...
                            if (sortByValue == '${local?.optionAllTime}') {
                              setState(() {
                                isOneDay = false;
                                multiday = false;
                                now = false;
...
// calendar icon on appbar
  IconButton _calendarIcon(BuildContext context) {
    return IconButton(
        onPressed: () {
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (_) => AttendanceCalendar(id: userDisplay[0].id!)));
        },
        icon: const Icon(Icons.calendar_today));
  }

// attendance list with listview and expansiontile widget
  Expanded _attendanceList(bool isEnglish, AppLocalizations? local) {
    return Expanded(
      child: Column(
        children: [
          SizedBox(
          ...
          Container(
            padding:
                const EdgeInsets.only(left: 22, right: 32, top: 12, bottom: 12),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Row(
                  children: [
                    Text(
                      isEnglish ? '${local?.attendance} ' : '${local?.list} ',
                      style: kHeadingThree,
                    ),
                    Text(
                      isEnglish ? '${local?.list} ' : '${local?.attendance} ',
                      style: kHeadingThree,
                    ),
                  ],
                ),
                  ...
                  : Expanded(
                      child: Padding(
                        ...
                        child: AttendanceInfoAttendanceList(
                          ...
                          attendancesByIdDisplay: attendancesByIdDisplay,
                          attendanceAll: _attendanceAll,
                          fetchAttedancesById: fetchAttedancesById,
                          fetchAllAttendance: fetchAllAttendance,
                          fetchAttendanceCount: fetchAttendanceCount,
                          fetchAttendanceCountAll: fetchAttendanceCountAll,
                        ),
                      ),
                    ),
        ],
          ...
          Row(
            ...
            children: [
              AttendanceInfoPresent(
                ...
                now: now,
                todayMorning: attendanceCount!.morningPresent.toString(),
                todayAfternoon: attendanceCount!.afternoonPresent.toString(),
                isLoading: _isLoading,
                isOneday: isOneDay,
                onedayMorning: attendanceCount!.morningPresent.toString(),
                onedayAfternoon: attendanceCount!.afternoonPresent.toString(),
                presentAll: attendanceCountAll!.totalPresent.toString(),
                alltime: alltime,
                text: '${local?.present} ',
                afternoon: afternoon,
                multipleDay: multiday,
                presentAfternoon: attendanceCount!.afternoonPresent.toString(),
                presentMorning: attendanceCount!.morningPresent.toString(),
              ),
              AttendanceInfoPresent(
                total: total,
                isLoadingAll: _isLoadingAll,
              ...
              AttendanceInfoPresent(
                ...
                now: now,
                todayMorning: attendanceCount!.morningLate.toString(),
                todayAfternoon: attendanceCount!.afternoonLate.toString(),
                isLoading: _isLoading,
                isOneday: isOneDay,
                onedayMorning: attendanceCount!.morningLate.toString(),
                onedayAfternoon: attendanceCount!.afternoonLate.toString(),
                presentAll: attendanceCountAll!.totalLate.toString(),
                alltime: alltime,
                text: '${local?.late} ',
                afternoon: afternoon,
                multipleDay: multiday,
                presentAfternoon: attendanceCount!.afternoonLate.toString(),
                presentMorning: attendanceCount!.morningLate.toString(),
              ),
              AttendanceInfoPresent(
                total: total,
                isLoadingAll: _isLoadingAll,
...
// fetching and loading widget
  Container _fetchData(AppLocalizations? local) {
    return Container(
