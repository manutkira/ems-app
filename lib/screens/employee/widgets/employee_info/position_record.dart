                      ...
                      child: Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            _positionNameInput(isEnglish, local, context),
                            SizedBox(
                              height: isEnglish ? 15 : 0,
                            ),
                            _startDateInput(local, context),
                            const SizedBox(
                              height: 20,
                            ),
                            _endDateInput(local, context),
                            const SizedBox(
                              height: 20,
                            ),
      ...
      body: _isLoading
          ...
          : ListView(
              children: [
                ...
                _profileImgAndNameID(context, local),
                const SizedBox(
                  height: 25,
                ),
                _postionTextAndIcon(local),
                !table
                    ? ListView.builder(
                        // reverse: true,
                        shrinkWrap: true,
                        ...
                        itemBuilder: (BuildContext contexxt, int index) {
                          ...
                          return Stack(
                            children: [
                              _popupMenu(index, context, local, isEnglish),
                              _startDate(),
                              _positionNameFromTo(index, local, isEnglish),
                              _blueLine(),
                              _redDot()
                            ],
                          );
                        ...
                        child: Table(
                          ...
                          children: positionsDisplay.map<TableRow>((e) {
                            ...
                            return TableRow(children: [
                              _columnPositionName(e),
                              _columnStartAndEndDate(local, isEnglish),
                              _columnPopupmenu(e, context, local, isEnglish)
                            ]);
                          }).toList(),
                        ),
                      )
...
// column with popupmenu for edit and delete position
        ...
        onSelected: (int selectedValue) async {
          if (selectedValue == 0) {
            ...
            startDateController.text = DateFormat('dd-MM-yyyy')
                .format(DateTime.tryParse(e.startDate.toString())!);
            endDateController.text = e.endDate == null
                ? 'Null'
                : DateFormat('dd-MM-yyyy')
                    .format(DateTime.tryParse(e.endDate.toString())!);
            pickStart = DateTime.tryParse(e.startDate.toString());
            if (e.endDate != null) {
              pickEnd = DateTime.tryParse(e.endDate.toString());
            }
            await showModalBottomSheet(
                context: context,
                builder: (_) {
...
// column with start and end date of positions
  TableCell _columnStartAndEndDate(AppLocalizations local, bool isEnglish) {
    return TableCell(
        child: Padding(
...
// column with position's name
  TableCell _columnPositionName(Position e) {
    return TableCell(
        verticalAlignment: TableCellVerticalAlignment.middle,
        child: Padding(
...
// container with position name and start date and end date
  Padding _positionNameFromTo(
    int index,
    AppLocalizations local,
    bool isEnglish,
  ) {
...
// start date at the left screen
  Positioned _startDate() {
    return Positioned(
      top: 50.0,
      left: 11.0,
      child: Text(
...
// popupmenu for edit and delete position
  Positioned _popupMenu(
    int index,
    BuildContext context,
    AppLocalizations local,
    bool isEnglish,
      ...
      child: PopupMenuButton(
        ...
        onSelected: (int selectedValue) async {
          if (selectedValue == 0) {
            ...
            positionName.text = positionsDisplay[index].name.toString();
            startDateController.text = DateFormat('dd-MM-yyyy').format(
                DateTime.tryParse(
                    positionsDisplay[index].startDate.toString())!);
            endDateController.text = positionsDisplay[index].endDate == null
                ? 'Null'
                : DateFormat('dd-MM-yyyy').format(DateTime.tryParse(
                    positionsDisplay[index].endDate.toString())!);
            pickStart =
                DateTime.tryParse(positionsDisplay[index].startDate.toString());
            if (positionsDisplay[index].endDate != null) {
              pickEnd =
                  DateTime.tryParse(positionsDisplay[index].endDate.toString());
            }
            await showModalBottomSheet(
                shape: const RoundedRectangleBorder(
                    borderRadius: BorderRadius.only(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          _positonInputEdit(local, isEnglish, context),
                          _startDateEdit(local, isEnglish, context),
                          _endDateEdit(local, isEnglish, context),
                          const SizedBox(
                            height: 20,
                          ),
                          _yesAndNoBtnEdit(context, local),
                        ],
                      ),
                    ),
                  );
...
// yes/no button for updating position or not
  Padding _yesAndNoBtnEdit(BuildContext context, AppLocalizations local) {
    return Padding(
      padding: const EdgeInsets.only(right: 30),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.end,
...
// end date input for edit positon
  Padding _endDateEdit(
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Padding(
            padding: const EdgeInsets.only(left: 25, right: 5, bottom: 15),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(
...
// start date input for edit position
  Padding _startDateEdit(
      AppLocalizations local, bool isEnglish, BuildContext context) {
    return Padding(
      padding: const EdgeInsets.all(0),
      child: Row(
...
// postion name input for edit position
  Padding _positonInputEdit(
      AppLocalizations local, bool isEnglish, BuildContext context) {
    return Padding(
      padding: const EdgeInsets.all(0),
      child: Row(
...
// position record text and icon for viewing timeline/table
  Padding _postionTextAndIcon(AppLocalizations local) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 30, left: 10, right: 20),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(
  ...
  Padding _profileImgAndNameID(BuildContext context, AppLocalizations local) {
    return Padding(
      ...
      child: Container(
        ...
        padding:
            const EdgeInsets.only(top: 20, bottom: 20, right: 16, left: 30),
...
// name and id
  Column _nameAndId(AppLocalizations local) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        BaselineRow(
...
// yes/no button for saving new position or not
  Padding _yesAndNoBtn(BuildContext context, AppLocalizations local) {
    return Padding(
      padding: const EdgeInsets.only(right: 30),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.end,
...
// end date input field for new position
  Padding _endDateInput(AppLocalizations local, BuildContext context) {
    return Padding(
...
// start date input field for new position
  Padding _startDateInput(AppLocalizations local, BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(right: 30, left: 20),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
...
// position name for new postion
  Row _positionNameInput(
    bool isEnglish,
    AppLocalizations local,
    BuildContext context,
  ) {
